...
def queryProlog(diseaseList):
    prolog = PrologMT()
    for disease in diseaseList:
        cadena = f"yes({disease})"
        prolog.assertz(cadena)
    
    prolog.consult("./baseConocimiento/hechos.pl")
    prolog.consult("./baseConocimiento/reglas.pl")
    prolog.consult("./resources/call_functions.pl")
    
    c = prolog.query("tiene(Enfermedad).")
    soluciones = list(c)
    return soluciones
